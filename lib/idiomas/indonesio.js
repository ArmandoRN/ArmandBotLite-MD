...
const smsRowner = () => { return `\`\`\`*ERROR*\`\`\`` }
const smsOwner = () => { return `\`\`\`*ERROR*\`\`\`` }
...
const smsBotAdmin = () => { return `\`\`\`¡¡NECESITO SER ADMIN PARA USAR ESTE COMANDO!!\`\`\`` }
const smsUnreg = () => { return `\`\`\`¡¡NECESITAS ESTAR REGISTRADO PARA USAR ESTE COMANDO, ESCRIBE #verificar PARA REGISTRARTE!!\`\`\`` }
const smsRestrict = () => { return `\`\`\`*ERROR*\`\`\`` }

//main.js
const smsWelcome = () => { return `Hola`}
const smsBye = () => { return '┊ *'}
...
const smsSdesc = () => { return '*NUEVA DESCRIPCIÓN DEL GRUPO:*\n\n@desc'}
...
/*
//info-contacto.js
const smsContacto1 = () => { return ' Soy ' + packname + ' un Bot de WhatsApp'}
const smsContacto2 = () => { return 'Dueño de ' + packname + ' sí tienes alguna duda me la puedes decir'}
...
//descargas.js
const smsTiktok = () => { return 'DESCARGANDO TIKTOK'}
...
//config.js
const smsWait = () => { return '*espere un momento...*'}
...
//sticker-sticker.js
const smsSticker1 = (usedPrefix, command) => { return `*RESPONDER A UN VÍDEO, IMAGEN, y ESCRIBA ${usedPrefix + command} *`}
